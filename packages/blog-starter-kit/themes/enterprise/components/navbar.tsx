    ...
    const promises = images.map(
      (src) =>
        new Promise<void>((resolve, reject) => {
          ...
          img.src = src;
          img.onload = () => resolve();
          img.onerror = (error: Event | string) => reject(error instanceof Event ? error : new Error(error));
        }),
    );
...
return (
    <>
      <nav
        ref={navbarRef}
        className={`container fixed left-0 right-0 top-0 z-50 mx-auto w-full select-none px-4 py-4 pt-7 sm:pt-7 transition-all duration-500 ${
