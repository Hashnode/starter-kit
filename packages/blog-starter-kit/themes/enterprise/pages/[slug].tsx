...
const manualCategories = [
  { slug: 'kedi', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Kedi' },
  { slug: 'kopek', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Köpek' },
  { slug: 'kedi-bakimi', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Kedi Bakımı' },
  { slug: 'kedi-sagligi', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Kedi Sağlığı' },
  { slug: 'kedi-irklari', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Kedi Irkları' },
  { slug: 'kedi-beslenmesi', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Kedi Beslenmesi' },
  { slug: 'kedi-diger', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Kedi Diğer' },
  { slug: 'kopek-sagligi', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Köpek Sağlığı' },
  { slug: 'kopek-bakimi', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Köpek Bakımı' },
  { slug: 'kopek-irklari', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Köpek Irkları' },
  { slug: 'kopek-beslenmesi', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Köpek Beslenmesi' },
  { slug: 'kopek-diger', publishedAt: '2024-06-06T21:00:00.000Z', name: 'Köpek Diğer' },
];
...
const Post = ({ publication, post, relatedPosts }: PostProps) => {
  ...
  const mainCategory = post.tags?.find(tag => tag.name === "Kedi" || tag.name === "Köpek")?.name || "Genel";
  const subCategory = manualCategories.find(cat => 
    post.tags?.some(tag => tag.slug === cat.slug && cat.slug !== 'kedi' && cat.slug !== 'kopek')
  )?.name || "Diğer";
...
const Category = ({ series, posts, publication }: CategoryProps) => {
  const categoryInfo = manualCategories.find(cat => cat.slug === series.slug);
  const title = categoryInfo ? `${categoryInfo.name} - ${publication.title}` : `${series.name} - ${publication.title}`;
  ...
  return (
    <>
      ...
      <h1 className="text-5xl text-gray-900 font-semibold mt-2 mb-5">
          {categoryInfo ? categoryInfo.name : series.name}
        </h1>
        {posts.length > 0 ? (
          <MorePosts 
            context="series" 
            ...
            seriesName={categoryInfo ? categoryInfo.name : series.name}
          />
        ) : (
...
export const getStaticProps: GetStaticProps<Props, Params> = async ({ params }) => {
  ...
  if (seriesData.publication?.series) {
    ...
    // Manuel kategori bilgisini ekle
    const categoryInfo = manualCategories.find(cat => cat.slug === slug);
    ...
    return {
      props: removeUndefined({
        ...
        series: {
          ...series,
          name: categoryInfo ? categoryInfo.name : series.name,
        },
        posts,
