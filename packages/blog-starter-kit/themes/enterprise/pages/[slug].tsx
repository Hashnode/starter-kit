...
const Post = ({ publication, post }: PostProps) => {
	...
	const tagsList = (post.tags ?? []).map((tag) => (
		<li key={tag.id}>
			<Link
				href={`/tag/${tag.slug}`}
				className="block rounded-full border px-2 py-1 text-sm font-medium hover:bg-slate-50 dark:border-neutral-800 dark:hover:bg-neutral-800 md:px-4"
			>
				#{tag.slug}
			</Link>
		</li>
	...
	return (
		<>
			...
			{(post.tags ?? []).length > 0 && (
				<div className="w-full px-5 text-slate-600 dark:text-neutral-300 md:max-w-screen-md">
