...
const fontRegular = fetch(new URL('../../../assets/PlusJakartaSans-Regular.ttf', import.meta.url)).then((res) =>
  res.arrayBuffer(),
);

const fontMedium = fetch(new URL('../../../assets/PlusJakartaSans-Medium.ttf', import.meta.url)).then((res) =>
  res.arrayBuffer(),
);

const fontSemiBold = fetch(new URL('../../../assets/PlusJakartaSans-SemiBold.ttf', import.meta.url)).then((res) =>
  res.arrayBuffer(),
);
...
const fontExtraBold = fetch(new URL('../../../assets/PlusJakartaSans-ExtraBold.ttf', import.meta.url)).then((res) =>
  res.arrayBuffer(),
);

export default async function (req: NextRequest) {
  const [fontDataRegular, fontDataMedium, fontDataSemiBold, fontDataBold, fontDataExtraBold] = await Promise.all([
    fontRegular,
    fontMedium,
    fontSemiBold,
    fontBold,
      ...
      >
        ...
        >
          ...
          <div tw="flex flex-1 flex-col items-center justify-center">
            ...
            <p tw={`${titleTailwindClass} mb-5 mt-0 text-center font-extrabold leading-tight`}>{title}</p>
          </div>
