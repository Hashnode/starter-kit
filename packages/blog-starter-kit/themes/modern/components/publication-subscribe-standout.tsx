...
function PublicationSubscribeStandOut() {
  ...
  return (
    <div className="my-10 mt-20 flex w-full flex-col items-center pb-10 md:px-5">
      <h3 className="mb-5 text-center font-heading text-2xl font-bold text-slate-50 md:text-3xl">
        Subscribe to {publication!.isTeam ? 'our' : 'my'} newsletter
      </h3>
      <p className="mb-5 text-center text-lg text-slate-300 md:w-2/3 md:text-xl">
      ...
      {!state.subscribed && (
        <div className="flex flex-row overflow-hidden rounded-lg border  border-slate-200 md:w-2/3">
          <input
            type="email"
            ref={email}
            onKeyUp={handleEmailChange}
            placeholder="Enter your email address"
            className="w-full bg-transparent p-3  outline-none text-white md:p-5 md:text-lg"
          />
          <Button
            type="button"
            onClick={subscribe}
            ...
            className="shrink-0 rounded-none  px-3 font-bold uppercase tracking-wide  bg-slate-200 text-black hover:bg-slate-300 md:px-5"
          >
            Subscribe
          </Button>
        </div>
      ...
      {state.subscribed && (
        <div className="flex flex-col items-center overflow-hidden rounded-lg border  p-5 text-center  border-green-400 bg-green-900 text-white md:w-2/3">
